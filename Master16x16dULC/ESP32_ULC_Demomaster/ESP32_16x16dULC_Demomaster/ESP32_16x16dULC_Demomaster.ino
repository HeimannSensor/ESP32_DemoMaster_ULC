...
void Startup_Routine_Module(void) {
  ...
  Read_Module((uint8_t)CMD_READMASK, (uint8_t*)&pixel_mask[0][0], PIXEL_PER_COLUMN * sizeof(pixel_mask[0][0])); //CK: changed 8 to PIXEL_PER_COLUMN * sizeof(pixel_mask[1][0])
  Read_Pixelmask();
  delay(10);
...
void Write_Pixelmask(void) {
  ...
  Write_Module((uint8_t)CMD_WRITEMASK, (uint8_t*)&pixel_mask[1][0], PIXEL_PER_COLUMN * sizeof(pixel_mask[1][0]));  //CK: changed 8 to PIXEL_PER_COLUMN * sizeof(pixel_mask[1][0])
  delay(500);
  return;
}
...
void Read_Pixelmask(void) {  
    ...
    for(i=0;i<PIXEL_PER_COLUMN;i++){                                                           //CK: changed 8 to PIXEL_PER_COLUMN
      for(k=0;k<PIXEL_PER_COLUMN;k++){                                                         //CK: changed 8 to PIXEL_PER_COLUMN
        Serial.print((pixel_mask[0][i]>>((PIXEL_PER_COLUMN - 1)-k))&0x1);       //CK: changed 7 to (PIXEL_PER_COLUMN - 1)
        if((pixel_mask[0][i]>>((PIXEL_PER_COLUMN - 1)-k))&0x1)                  //CK: changed 7 to (PIXEL_PER_COLUMN - 1)
          nrOfActPixel++;
      }
      Serial.print("\r\n");
    }
...
void Set_Pixel_Mask(void) {
  ...
  for (i = 0; i < PIXEL_PER_COLUMN; i++)                                              //CK: changed 8 to PIXEL_PER_COLUMN
    #ifdef HTPA_8x8d
    pixel_mask[1][i] = (uint8_t)(packetBuffer[(PIXEL_PER_COLUMN * 2 + 1) + 2 * i] - '0'); // expecting comma separated bytes reprenting pixels per bit (1 = activ, 0 = inactive pixel)  //CK:  changed 17 to (PIXEL_PER_COLUMN * 2 + 1)
    #else if HTPA_16x16d
    pixel_mask[1][i] = (uint16_t)(packetBuffer[(PIXEL_PER_COLUMN * 2 + 1) + 2 * i] - '0'); // expecting comma separated bytes reprenting pixels per bit (1 = activ, 0 = inactive pixel)  //CK:  changed 17 to (PIXEL_PER_COLUMN * 2 + 1)  
    #endif

  // retransform bit-wise encoding to local byte-wise handling
  for (i = 0; i < PIXEL_PER_COLUMN; i++) {                                                             //CK: changed 8 to PIXEL_PER_COLUMN
    if (pixel_mask[1][i]){
        for(j = 0; j < PIXEL_PER_COLUMN; j++){                                                         //CK: changed 8 to PIXEL_PER_COLUMN
...
void SortUDPpacket(void) {
  ...
  for (i = 0; i < (PIXEL_PER_COLUMN * PIXEL_PER_ROW)/2; i++) {
    packet1[bytei++] = 0;
    packet1[bytei++] = 0;
  }
...
void CheckSerial(void) {
  ...
  switch (serial_input) {
    ...
    case '8':  //reset pixel mask to all pixel
      ...
      TimerLib.clearTimer();
      #ifdef HTPA8x8d
      memset((void*)&pixel_mask[0],0xFF,PIXEL_PER_COLUMN);                              //CK: changed 8 to PIXEL_PER_COLUMN
      #else if HTPA16x16d
      memset((void*)&pixel_mask[0],0xFF,PIXEL_PER_COLUMN*2);                              //CK: changed 8 to PIXEL_PER_COLUMN
      #endif
      Write_Module((uint8_t)CMD_WRITEMASK, (uint8_t*)&pixel_mask[0], PIXEL_PER_COLUMN * sizeof(pixel_mask[1][0])); //CK: changed 8 to PIXEL_PER_COLUMN * sizeof(pixel_mask[1][0])
      delay(120);
      Read_Pixelmask();
      readingRoutineEnable = true;
      TimerLib.setInterval_us(ISR, timert);

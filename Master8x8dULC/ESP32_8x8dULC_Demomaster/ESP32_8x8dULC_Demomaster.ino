...
//uint8_t pixel_mask[2][8] = {{0, 0, 0, 0, 0, 0, 0, 0}, {0x01, 0x03, 0x06, 0x0C, 0x18, 0x30, 0x60, 0xC0}}; // [0][..] contains mask read from sensor module, [1][..] contains correct mask on master side  //diagonal from upper right to lower left corner
uint8_t pixel_mask[2][8] = {{0, 0, 0, 0, 0, 0, 0, 0}, {0x08, 0x08, 0x08, 0x08, 0x08, 0x08, 0x08, 0x08}}; // [0][..] contains mask read from sensor module, [1][..] contains correct mask on master side //vertical, 5th column
//uint8_t pixel_mask[2][8] = {{0, 0, 0, 0, 0, 0, 0, 0}, {0x01, 0x03, 0x07, 0x0F, 0x1F, 0x3F, 0x7F, 0xFF}}; // [0][..] contains mask read from sensor module, [1][..] contains correct mask on master side
//uint8_t pixel_mask[2][8] = {{0, 0, 0, 0, 0, 0, 0, 0}, {0x0f, 0x1F, 0x3F, 0x7F, 0xFF, 0xFF, 0xFF, 0xFF}}; // [0][..] contains mask read from sensor module, [1][..] contains correct mask on master side
uint8_t nrOfActPixel = 15; //8
uint32_t sensorID = 0;
uint8_t epsilon = 100;
uint16_t ta = 0;
...
char serial_input = 'm';
//uint16_t baudrate_serial = 115200;
...
void IRAM_ATTR ISR(void){
  ...
  if (readingRoutineEnable)
    checkForNewData = true; //CK: DEBUG ONLY
...
void Print_Menu(void) {
  Serial.println("\n\n\n***************************************************");
  Serial.println("8x8ULC Demomaster v1.0                  /_/eimann");
  ...
  Serial.println("  9... send device into deep sleep");
...
void Read_Temp_Frame(void) {
  ...
  if(noEOCcounter >= NR_OF_MAX_POLLS){
    ...
    Read_Module(CMD_READDATA, (uint8_t*)&dummyBuffer, (uint16_t)(nrOfActPixel * 2 + 2)); // +2 for sensor Tamb

    // sort according to current active-pixel-mask
    for(i = 0; i < PIXEL_PER_COLUMN; i++){
      for(j = 0; j < PIXEL_PER_ROW; j++){
        if((pixel_mask[0][i]>>(j))&0x1){
          pixel_data[i*8+(j)] = dummyBuffer[nrPixel];
          nrPixel++;
        }
      else
        pixel_data[i*8+(j)] = 0;
...
void Startup_Routine_Module(void) {
  ...
  #ifdef INITIAL_MASK_HI
    memset((void*)&pixel_mask[0][0],0xFF,8);
    memset((void*)&pixel_mask[1][0],0xFF,8);
    Serial.println("Set Pixel mask high via INITIAL_MASK_HI");
  #endif
  // wake up module
  ...
  Read_Module((uint8_t)CMD_READMASK, (uint8_t*)&pixel_mask[0][0], 8);
  Read_Pixelmask();
  delay(10);
  // check if read pixel mask is correct and wirte correct one if not so
  if (!Check_Pixelmask())
  //  Write_Pixelmask();
  delay(10);
...
/********************************************************************
   Function:        Check_Pixelmask()
   Description:     compare master's and slave's current pixel mask
 *******************************************************************/
bool Check_Pixelmask(void) {
  uint8_t i = 0;

  for (i = 0; i < 8; i++) {
    if (pixel_mask[0][i] != pixel_mask[1][i])
      return false;
  }
...
/********************************************************************
   ...
   Description:     encode local active-pixel-mask and write to 8x8dULD module
 *******************************************************************/
void Write_Pixelmask(void) {
  // write new mask to module (bit-wise)
  Write_Module((uint8_t)CMD_WRITEMASK, (uint8_t*)&pixel_mask[1][0], 8);
  delay(500);
  return;
}
...
void Read_Pixelmask(void){
    ...
    unsigned char i,k;
    uint8_t temp;
    Read_Module((uint8_t)CMD_READMASK, (uint8_t*)&pixel_mask[0], 8);
    nrOfActPixel=0;

    for(i=0;i<8;i++){
      for(k=0;k<8;k++){
        Serial.print((pixel_mask[0][i]>>(7-k))&0x1);
        if((pixel_mask[0][i]>>(7-k))&0x1)
          nrOfActPixel++;
      }
      Serial.print("\r\n");
    }
...
/********************************************************************
   Function:        void Set_Pixel_Mask(void)
...
void Set_Pixel_Mask(void) {
  ...
  for (i = 0; i < 8; i++)
    pixel_mask[1][i] = (uint8_t)(packetBuffer[17 + 2 * i] - '0'); // expecting comma separated bytes reprenting pixels per bit (1 = activ, 0 = inactive pixel)
...
void CheckSerial(void) {
  ...
  switch (serial_input) {
    ...
    case 'f':
      readingRoutineEnable = false;
      Read_Module((uint8_t)CMD_READFWV, (uint8_t*)&fwv, 1);
    ...
    case '8':  //rest pixel mask to all pixel
      readingRoutineEnable = false;
      TimerLib.clearTimer();
      memset((void*)&pixel_mask[0],0xFF,8);
      Write_Module((uint8_t)CMD_WRITEMASK, (uint8_t*)&pixel_mask[0], 8);
      delay(120);
      Read_Pixelmask();
      readingRoutineEnable = true;
      TimerLib.setInterval_us(ISR, timert);
...
case '9':
      ...
      TimerLib.clearTimer();
      // wake up module
      Write_Module((uint8_t)CMD_DEEPSLEEP, (uint8_t*)&setting, 0);  //allow TBD ms delay at wakeup from deep sleep
      delay(10);
